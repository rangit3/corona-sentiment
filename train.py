    ...
    with torch.no_grad():
        for batch in tqdm(generator):
         try:
            ...
            #loss += error.item()
            pred_labels = torch.argmax(logits, axis=1)
            print(str(pred_labels)+" "+str(batch))
            labs=np.concatenate([labs,pred_labels.cpu().numpy()])
            #acc += (pred_labels == labels).sum().item()
         except:
            continue
    np.savetxt('filename.csv', labs, delimiter=",")
    loss /= len(dataset)
    acc /= len(dataset)
    return loss, acc
...
def train(
    ...
    model=torch.load('bert-large-uncased__all__fine__e5.pickle')
