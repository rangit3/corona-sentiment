...
class SSTDataset(Dataset):
    ...
    def __init__(self, split="train", root=True, binary=True):
        ...
        #self.sst = sst[split]
        df=pd.read_csv('~/corona/bert_sentiment/data2.csv')
        ...
        if root and binary:
            ...
            self.data = [
                (rpad(tokenizer.encode("[CLS] " + line + " [SEP]"), n=66), [1])
                for tree in df.values
                for line in re.split('[?.,]', tree[5]) if 'zoxanide' in line
            ]
